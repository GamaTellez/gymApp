...
//    }
    ...
    NSArray *arrayWithFavExercises = [[NSArray alloc] initWithArray:[[ModelController sharedInstance] maxWeightOfFavoriteExercisesWithName:self.exercise.exerciseName]];
    NSMutableArray *dataModels = [[NSMutableArray alloc] init];
    int i = 0;
    for (Exercise *exercise in arrayWithFavExercises) {
        ELineChartDataModel *newDataModel =[[ELineChartDataModel alloc] initWithLabel:[NSString stringWithFormat:@"%@",exercise.maxWeight] value:[exercise.maxWeight floatValue] index:i unit:@"Lbs"];
        i++;
        [dataModels addObject:newDataModel];
        NSLog(@"%@", dataModels);
    }

    _eLineChartData = [NSArray arrayWithArray:dataModels];
